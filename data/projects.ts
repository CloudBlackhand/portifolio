...
export const projects: Project[] = [
  ...
  },
  {
    ...
    title: "VionHub — gestão comercial confidencial",
    shortDescription:
      "CRM de vendas em produção no Railway: funil completo, financeiro, documentos, equipe com papéis e pós-venda via WhatsApp — dados reais sob sigilo.",
    detailedDescription:
      "O VionHub (repositório privado VendasHub) é o ecossistema comercial que centraliza a operação de vendas de telecomunicações: cadastro unificado de oportunidades com dezenas de campos, aprovação em backoffice, dashboard comparando períodos, visão por operadora e upload automático de documentos organizados por tipo e operadora. Usuários, logs e permissões ficam em PostgreSQL; o pipeline de vendas roda em planilha sincronizada para edição operacional; arquivos vão para armazenamento em nuvem com pastas por operadora. Há módulos de financeiro, RV, antecipação semanal, sucesso do cliente com disparos WhatsApp templated, solicitações internas, grupos com permissões granulares e mapa em tempo real. Quatro papéis (desenvolvedor, admin, backoffice, vendedor) controlam quem vê financeiro, quem aprova venda e quem dispara mensagem. Por lidar com CPF, endereço, contratos e pipeline comercial real, o case no portfólio é só descritivo: prova entrega em produção sem expor telas, IDs de integração, métricas ou identidade do contratante.",
    thumbnail: "/project-thumbs/topsecret-comercial.svg",
    category: "Top Secret",
    year: 2025,
    impactLabel: "Operação comercial em produção",
    featured: false,
    confidential: true,
    stack: [
      "Node.js",
    ...
    context:
      "A operação comercial precisava sair de planilhas soltas e grupos de mensagem: um funil único, rastreável, com documentos, financeiro e pós-venda integrados — sem permitir vazamento de dados de clientes ou pipeline para fora do ambiente autorizado.",
    challenges: [
      "Sincronizar cadastro rico de vendas entre interface web, planilha operacional e armazenamento de documentos sem perder consistência.",
      "Separar permissões entre vendedor, backoffice, financeiro e sucesso do cliente — inclusive quem pode enviar WhatsApp operacional.",
      "Manter sistema estável em produção na nuvem com autenticação, recuperação de senha e notificações em tempo real.",
      "Documentar capacidade técnica sem publicar capturas, URLs, webhooks, IDs de planilha ou qualquer dado identificável.",
    ],
    solution: [
      "Backend REST com autenticação JWT, roles explícitas e serviços dedicados para planilha, drive e mensageria.",
      "Frontend responsivo com dashboard analítico, grid paginado server-side, modal único de venda e módulos financeiro / RV / antecipação.",
      "Integração WhatsApp via camada WAHA para sucesso do cliente e rotinas operacionais, com sessões controladas por permissão.",
      "Deploy contínuo em Railway; portfólio limitado ao que o acordo de sigilo permite — contexto, abordagem e resultado.",
    ],
    results: [
      "Funil comercial, documentos e financeiro rodando no mesmo ambiente — menos retrabalho entre vendedor, backoffice e pós-venda.",
      "Visibilidade interna de pipeline e desempenho sem depender de export manual diário.",
      "Dados de clientes, contratos e identidade da operação preservados fora do site público.",
      "Referência sólida para quem precisa de CRM vertical com integrações Google e mensageria sob confidencialidade.",
    ],
  },
  {
    slug: "topsecret-sales-hub",
    projectKind: "topsecret",
    title: "Sales Hub — CRM multiempresa confidencial",
    shortDescription:
      "CRM multi-tenant genérico em Next.js: empresas isoladas por slug, vendas, clientes, comissões, financeiro e pós-venda — repositório privado Sales-Hub.",
    detailedDescription:
      "O Sales Hub é a evolução arquitetural do ecossistema comercial: CRM web multiempresa onde cada organização opera em ambiente isolado (slug próprio), com papéis de owner, admin, gerente e vendedor. Cobre cadastro de produtos e clientes, registro de vendas, comissões por vendedor, transações financeiras, atividades de pós-venda, overview com filtros de período e painel de supervisão global. Autenticação persistente com Better Auth, banco PostgreSQL modelado em Prisma, UI moderna com componentes acessíveis e gráficos analíticos. Pode rodar em modo demonstração somente leitura para apresentação comercial sem gravar dados sensíveis. Repositório privado no GitHub; deploy preparado para Railway com migrações no arranque. O dossiê descreve o produto e a stack — nunca CNPJs, credenciais, empresas reais ou capturas de operação.",
    thumbnail: "/project-thumbs/vendas-hub.svg",
    category: "Top Secret",
    year: 2026,
    impactLabel: "CRM reutilizável multi-tenant",
    featured: false,
    confidential: true,
    stack: [
      "Next.js",
    ...
    context:
      "Depois de entregar CRM vertical em produção, surgiu a necessidade de um produto comercial genérico — escalável por empresa, sem acoplar a planilhas externas, pronto para diferentes segmentos.",
    challenges: [
      "Modelar multi-tenant com membros, vendedores e permissões sem vazamento cruzado entre empresas.",
      "Unificar vendas, comissões, financeiro e pós-venda num fluxo coerente substituindo planilhas.",
      "Permitir demo pública ou apresentação sem expor gravações reais de clientes.",
    ],
    solution: [
      "Schema Prisma com Company, Sale, Commission, Customer, PostSaleActivity e FinancialTransaction.",
      "Rotas por slug de empresa, onboarding guiado e supervisão central para operadores autorizados.",
      "Flag de ambiente somente leitura para demonstrações; analytics opcional desacoplado.",
    ],
    results: [
      "Base CRM completa pronta para demo ou produção com cliente real.",
      "Arquitetura reutilizável — telecom, serviços ou qualquer operação comercial com equipe e comissões.",
      "Código e dados permanecem em repositório privado; portfólio mostra só o nível executivo.",
    ],
  },
  {
    slug: "topsecret-automacao-whatsapp",
    projectKind: "topsecret",
    title: "Evolution API — infra WhatsApp confidencial",
    shortDescription:
      "Instância self-hosted da Evolution API (evo-wp): REST HTTPS, instâncias Baileys/Cloud, webhooks, filas e integrações — base dos bots e disparadores em produção.",
    detailedDescription:
      "Camada de infraestrutura WhatsApp mantida em repositório privado (fork evo-wp da Evolution API v2): API REST em HTTPS para criar e gerenciar instâncias, conectar sessões WhatsApp Web (Baileys) ou Cloud API oficial, receber eventos via webhooks e integrar com bots, filas, storage e ferramentas de atendimento. Persistência em PostgreSQL, cache Redis, deploy containerizado na Railway. Esta camada alimenta produtos públicos do portfólio (como o bot de CEP e o disparador MS) e sistemas confidenciais — mas tokens de sessão, números conectados, payloads de conversa, chaves de API e endpoints internos nunca entram no material aberto. Em paralelo, o ecossistema comercial usa também WAHA para rotinas operacionais de pós-venda — duas pilhas complementares, mesma exigência de sigilo.",
    thumbnail: "/project-thumbs/sistema-https-whatsapp.svg",
    category: "Top Secret",
    year: 2025,
    impactLabel: "Infra mensageria sob sigilo",
    featured: false,
    confidential: true,
    stack: [
      "Evolution API",
    ...
    context:
      "Bots, disparadores e CRM precisavam de uma API WhatsApp estável, centralizada e controlada — não depender de serviços opacos nem espalhar sessões em cada produto.",
    challenges: [
      "Operar múltiplas instâncias e consumidores (bots, painéis, automações) sem conflito de sessão ou vazamento entre produtos.",
      "Proteger tokens, webhooks e conteúdo de conversa em ambiente de produção real.",
      "Manter fork atualizado, containerizado e observável sem publicar arquitetura interna no portfólio.",
    ],
    solution: [
      "Deploy self-hosted com autenticação por API key, instâncias nomeadas e eventos encaminhados por webhook.",
      "Separação clara entre infra (Evolution/WAHA) e produtos de negócio (públicos ou confidenciais).",
      "Tráfego exclusivamente HTTPS; credenciais restritas ao ambiente autorizado.",
    ],
    results: [
      "Canal WhatsApp previsível para atendimento automatizado, disparos e integrações corporativas.",
      "Base técnica comprovada para CEPBOT, MS e operações confidenciais — sem expor evidência sensível.",
      "Referência para projetos que exigem API própria, governança de instâncias e rigor de confidencialidade.",
    ],
  },
];
